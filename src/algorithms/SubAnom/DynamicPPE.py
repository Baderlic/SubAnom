...
#generate an empty list used numba
...
class GraphPPREstimator():
    ...
    def get_approx_ppv_with_new_edges_batch_multip_source(self, query_node_desc_list, E_t, bool_return_dict = True, track_mode ='all', top_index = 100):
        ...
        #export graph of former snapshot. to get neighbors of ego-node
        original_G = self.g
        D = self.node_degree
        ...
        s_id_list = numba.typed.List([np.uint32(0)])# empty list
        ...
        new_col=[]
        new_data=[]
        # do second sweep, update the edge into the graph,
        ...
        if bool_return_dict: # this is for debug only, access numba.typed.Dict in python mode is very slow.
            ...
            return s_id_list, _s_id_list, original_G
    ...
    def get_exact_ppv_dict(self, query_node_desc):
        ...
        #NOTE: We can't directly get the ppr of the multigraph, so just convert it to a simple graph
        H = convert_multi(self.G)
        ppr_dict = nx.pagerank(H, personalization={query_node_id:1}, alpha=1-self.alpha, weight = 'w' )
