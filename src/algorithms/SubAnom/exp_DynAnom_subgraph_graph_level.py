'''
exp_DynAnom_subgraph_graph_level.py
generate_subgraph:  using triadic closure
subgraph to node strategy: mean, sum, max, min, median
'''
...
# current:  PATH-TO/kdd22-anomaly/src/algorithms/SubAnom
...
sys.path.append(os_join(project_root,'src',"algorithms", "SubAnom"))
import argparse
import pprint
...
def search_neighbor(graph, node):
    return graph.indices[graph.indptr[node]:graph.indptr[node+1]]


def generate_egonet(graph, node, hop_k):
    nodes = search_neighbor(graph, node).tolist()
    dq1 = deque(nodes)
    dq2 = deque()
    for i in range(hop_k - 1):
        for node2 in dq1:
            dq_temp = search_neighbor(graph, node2)
            dq2.extend(dq_temp)
            #dq1.popleft()
        nodes += list(dq2)
        dq1.clear()
...
def triangle(graph, Scn, node):
    all_neighbors = search_neighbor(graph, node).tolist()
    two_step_neighbors = Scn.indices[Scn.indptr[node]:Scn.indptr[node+1]]
    return list(set(all_neighbors).intersection(two_step_neighbors))

def triangle_subgraph(graph, Scn, node):
    neighbors = triangle(graph, Scn, node)
    neighbors.append(node)
    return neighbors

def triangle_based_subgraph_no_weight(graph, Scn, node):
    neighbors_layer1 = search_neighbor(graph, node).tolist()
    strong_neighbors = triangle(graph, Scn, node)
    subgraph_nodes = neighbors_layer1
    for neighbor in strong_neighbors:
        neighbors_layer2 = search_neighbor(graph, neighbor).tolist()
        subgraph_nodes.extend(neighbors_layer2)
    return list(set(subgraph_nodes))
    # nodes in subgraph must meet 2 requirements: 1. triangle net
...
def test_graph_stream(  path, dataset_name, timeStep, initSS , \
                        ...
                        generate_subgraph, hop_k, subgraph2node_strategy, subgraph_pattern):
    # for output file format
    data_config_str = f'dataset_{dataset_name}-initSS_{initSS}-timeStep_{timeStep}_subgraph_pattern_{subgraph_pattern}'
    algo_config_str = f'DynAnomPy-alpha_{alpha:.2e}-epsilon_{epsilon:.2e}-track_mode_{track_mode}-dim-{dim}-top_index_{top_index}-push_threshold-{selective_push_threshold:.2e}-make_directed_graph_{make_directed_graph}-make_multi_graph_{make_multi_graph}-hop_k_{hop_k}-strategy_{subgraph2node_strategy}'
    ...
    attack = np.zeros(testNum+1) # the ground-truth of each snapshot
    ...
    detect_anomaly: object = detect_anomaly# False
    ...
    query_node_desc_set: Set[Any] = set([])
    ...
    # k-hop-neighbors are included in the subgraph of a node
    generate_subgraph = generate_subgraph
    hop_k = int(hop_k)

    strategies = ["mean", "sum", "max", "min"]

    dict_anomaly_scores = {}
    dict_anomaly_subgraph_scores = {}
    subgraph_len = []
    # for strategy in strategies:
    #     dict_anomaly_subgraph_scores[strategy] = {}
    ...
    def __assign_anomaly_subgraph_score(key, crt_snapshot, anom_score):
        if key not in  dict_anomaly_subgraph_scores.keys():
            dict_anomaly_subgraph_scores[key] = np.zeros(testNum+1)
        dict_anomaly_subgraph_scores[key][crt_snapshot - initSS] = anom_score


    # Anomaly score calculation
    def __anom_score(prev_appr, sp_appr, crt_snapshot, shared_nodes, union_nodes, curt_total_nodes, k_hop):
        ...
        return l1_sum_arr, l2_sum_arr, l1_sum_dynppe_arr, l2_sum_dynppe_arr

    #apply different strategies of subgraph anomaly to node
    def __anom_subgraph2node(arr, strategy="mean"):
        if strategy == "mean":
            return arr.mean()
        elif strategy == "sum":
            return arr.sum()
    ...
    def __anom_subgraph2node2graph(anom_score_arr, shared_nodes, node2subgraph, subgraph2node_strategy):
        strategy_list = []

        for node in shared_nodes:
            subgraph = node2subgraph[node]
            anom_subg_arr = anom_score_arr[subgraph]
            anom_node = __anom_subgraph2node(anom_subg_arr, subgraph2node_strategy)
            strategy_list.append(anom_node)
    ...
    #aggregate various anomaly score, pick out the most anomaly node
    def __anom_node2graph(l1_arr, l2_arr, l1_dynppe_arr, l2_dynppe_arr, crt_snapshot):
        if crt_snapshot >= initSS: # for those testing snapshot
            attack[crt_snapshot - initSS] = (attackNum >= attackLimit) # ground truth

            __assign_anomaly_subgraph_score("DynAnomSubgraphScore(l1-sum)", crt_snapshot, l1_arr.sum())
            __assign_anomaly_subgraph_score("DynAnomSubgraphScore(l1-mean)", crt_snapshot, l1_arr.mean())
            __assign_anomaly_subgraph_score("DynAnomSubgraphScore(l1-median)", crt_snapshot, np.median(l1_arr))
            __assign_anomaly_subgraph_score('DynAnomSubgraphScore(l1-max)', crt_snapshot, l1_arr.max())
            __assign_anomaly_subgraph_score('DynAnomSubgraphScore(l1-min)', crt_snapshot, l1_arr.min())

            __assign_anomaly_subgraph_score("DynAnomSubgraphScore(l2-sum)", crt_snapshot, l2_arr.sum())
            __assign_anomaly_subgraph_score("DynAnomSubgraphScore(l2-mean)", crt_snapshot, l2_arr.mean())
            __assign_anomaly_subgraph_score("DynAnomSubgraphScore(l2-median)", crt_snapshot, np.median(l2_arr))
            __assign_anomaly_subgraph_score('DynAnomSubgraphScore(l2-max)', crt_snapshot, l2_arr.max())
            __assign_anomaly_subgraph_score('DynAnomSubgraphScore(l2-min)', crt_snapshot, l2_arr.min())

            __assign_anomaly_subgraph_score("DynAnomSubgraphScore(l1-sum-ppe)", crt_snapshot, l1_dynppe_arr.sum())
            __assign_anomaly_subgraph_score("DynAnomSubgraphScore(l1-mean-ppe)", crt_snapshot, l1_dynppe_arr.mean())
            __assign_anomaly_subgraph_score("DynAnomSubgraphScore(l1-median-ppe)", crt_snapshot, np.median(l1_dynppe_arr))
            __assign_anomaly_subgraph_score('DynAnomSubgraphScore(l1-max-ppe)', crt_snapshot, l1_dynppe_arr.max())
            __assign_anomaly_subgraph_score('DynAnomSubgraphScore(l1-min-ppe)', crt_snapshot, l1_dynppe_arr.min())

            __assign_anomaly_subgraph_score("DynAnomSubgraphScore(l2-sum-ppe)", crt_snapshot, l2_dynppe_arr.sum())
            __assign_anomaly_subgraph_score("DynAnomSubgraphScore(l2-mean-ppe)", crt_snapshot, l2_dynppe_arr.mean())
            __assign_anomaly_subgraph_score("DynAnomSubgraphScore(l2-median-ppe)", crt_snapshot, np.median(l2_dynppe_arr))
            __assign_anomaly_subgraph_score('DynAnomSubgraphScore(l2-max-ppe)', crt_snapshot, l2_dynppe_arr.max() )
            __assign_anomaly_subgraph_score('DynAnomSubgraphScore(l2-min-ppe)', crt_snapshot, l2_dynppe_arr.min())

            if len(l1_arr) > 10:
                top10_mean_idx_1 = np.argpartition(l1_arr, -10)[-10:]
                top10_mean_1 = l1_arr[top10_mean_idx_1].sum()
                __assign_anomaly_subgraph_score('DynAnomScore(l1-max-top10)', crt_snapshot, top10_mean_1 )

                top10_mean_idx_2 = np.argpartition(l2_arr, -10)[-10:]
                top10_mean_2 = l2_arr[top10_mean_idx_2].sum()
                __assign_anomaly_subgraph_score('DynAnomScore(l2-max-top10)', crt_snapshot, top10_mean_2)

                top10_mean_idx_3 = np.argpartition(l1_dynppe_arr, -10)[-10:]
                top10_mean_3 = l1_dynppe_arr[top10_mean_idx_3].sum()
                __assign_anomaly_subgraph_score('DynAnomScore(l1-max-top10-ppe)', crt_snapshot, top10_mean_3 )

                top10_mean_idx_4 = np.argpartition(l2_dynppe_arr, -10)[-10:]
                top10_mean_4 = l2_dynppe_arr[top10_mean_idx_4].sum()
                __assign_anomaly_subgraph_score('DynAnomScore(l2-max-top10-ppe)', crt_snapshot, top10_mean_4 )
    ...
    with tqdm(range(len(snapshots))) as t:
        for loop_i, ss in enumerate(t):
            ...
            # #use sparse matrix to restore edges
            # col = []
            # row = []
            # weights = []
            ...
            # graph_matrix = csr_matrix(weights, (row, col))
            ...
            # _push_start = ptime()
            s_id_list, updated_s_id_list, prev_graph = estimator.get_approx_ppv_with_new_edges_batch_multip_source(query_node_desc_set, new_edges, bool_return_dict = False, track_mode =track_mode, top_index = top_index)
            ...
            ################################################################
            ####    Algorithm-specific process for subgraph generation
            ################################################################
            ...
            if detect_anomaly:
                ...
                if prev_appr is not None:
                    ...
                    l1_sum_arr, l2_sum_arr, l1_sum_dynppe_arr, l2_sum_dynppe_arr = __anom_score(prev_appr, sp_appr, snapshot, shared_nodes, union_nodes, curt_total_nodes, hop_k) # for graph level

                # generate subgraph of shared_nodes
                    if generate_subgraph:
                        # search edges of shared node
                        Scn = prev_graph * prev_graph
                        node2subgraph = {}
                        subgraph_ns = []
                        for selected_node in s_id_list:
                            if subgraph_pattern =="khop":
                                subgraph = generate_egonet(prev_graph, selected_node, hop_k)
                            if subgraph_pattern == "triangle":
                                subgraph = triangle_subgraph(prev_graph, Scn, selected_node)
                            if subgraph_pattern == "strong-all":
                                subgraph = triangle_based_subgraph_no_weight(prev_graph, Scn, selected_node)
                            #print(len(subgraph))
                            #subgraph_ns.append(len(subgraph))
                            node2subgraph[selected_node] = subgraph
                        #ubgraph_len.append(np.array(subgraph_ns).mean())
                        #shared_nodes_edges = new_edges_dict[shared_nodes_edges_id]

                        anom_strategies = {}

                        # print("length of sum arr:", len(l1_sum_arr), len(l1_sum_dynppe_arr))
                        l1_anom = __anom_subgraph2node2graph(l1_sum_arr, shared_nodes, node2subgraph, subgraph2node_strategy)
                        l2_anom = __anom_subgraph2node2graph(l2_sum_arr, shared_nodes, node2subgraph, subgraph2node_strategy)
                        l1_ppe_anom = __anom_subgraph2node2graph(l1_sum_dynppe_arr, shared_nodes, node2subgraph, subgraph2node_strategy)
                        l2_ppe_anom = __anom_subgraph2node2graph(l2_sum_dynppe_arr, shared_nodes, node2subgraph, subgraph2node_strategy)

                        __anom_node2graph(l1_anom, l2_anom, l1_ppe_anom, l2_ppe_anom, snapshot)
    ...
    ####
    ################################################################
    #print("the length of subgraph is", subgraph_len)
    ...
    if detect_anomaly:
        ...
        # print("scores:", dict_anomaly_subgraph_scores)
        anom_score_keys = list(dict_anomaly_subgraph_scores.keys())
        header_str = ','.join(anom_score_keys + ['Label'])
        anom_val_list = [ dict_anomaly_subgraph_scores[key].tolist() for key in anom_score_keys]
        ...
        for anom_score_key in anom_score_keys:
            dict_top_score_snapshot_id[anom_score_key] = np.argsort(dict_anomaly_subgraph_scores[anom_score_key])[::-1].tolist() # the snapshot id of highest detected anomaly score
    ...
    print(np.array(subgraph_ns).mean())
...
if __name__ == '__main__':
    ...
    parser.add_argument("-generate_subgraph", default=True,
                        help="generate the subgraph of a node")
    parser.add_argument("-hop_k", default=1,
                        help="subgraph of k-hop neighbors")
    parser.add_argument("-subgraph2node_strategy", default="mean",
                        help="aggregate method of anomaly score, from subgraph to node")
    parser.add_argument("-subgraph_pattern", default="khop",
                        help="the shape of ego-net, to aggregate anomaly score")
    ...
    dim = args.dim
    generate_subgraph = args.generate_subgraph
    hop_k = args.hop_k
    subgraph2node_strategy = args.subgraph2node_strategy
    subgraph_pattern = args.subgraph_pattern

    test_graph_stream(  data_path, dataset_name, timeStep, initSS, \
                        make_directed_graph, make_multi_graph, \
                        alpha, beta, epsilon, track_mode, top_index, dim, \
                        validate_appr, selective_push_threshold, \
                        detect_anomaly, write_snapshot_ppvs, generate_subgraph,\
                        hop_k, subgraph2node_strategy, subgraph_pattern)